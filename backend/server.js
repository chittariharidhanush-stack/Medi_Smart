...
function seed() {
  ...
  notifications.push({ id: id(), userId: patient.id, message: 'Welcome to Medi_Smart!', type: 'system', read: false, createdAt: now() });
}
seed();
...
function generateSVGQR(text) {
  // Generates a stylized clean vector QR representation SVG
  const size = 210;
  const padding = 15;
  const dots = [];
...
async function handle(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && path === '/api/sos') {
      ...
      notifications.push({ id: id(), userId: patient.id || 1, message: "Emergency SOS Alert Sent.", type: "emergency", read: false, createdAt: now() });
    ...
    // ==========================================
    // FEATURE 2: WEBRTC TELEMEDICINE (Signaling)
    // ==========================================
    if (req.method === 'POST' && path === '/api/consultation/signal') {
      ...
      const entry = { id: id(), type: data.type, payload: data.payload, sender: data.sender || 'peer', time: Date.now() };
      consultationSignals.get(room).push(entry);
      // Keep signal buffer small
    ...
    if (req.method === 'POST' && path === '/api/ai/chat') {
      ...
      if (query.includes('fever') || query.includes('temperature') || query.includes('cough') || query.includes('cold') || query.includes('throat')) {
        ...
        autoFill = {
          ...
          reason: "Mild to moderate fever with persistent cough and throat discomfort for 3 days.",
          diagnosis: "Acute Viral Upper Respiratory Infection (URI) with low-grade pyrexia.",
          treatment: "Rest, oral hydration. Paracetamol 500mg SOS for fever > 100°F. Review in 3 days if symptoms persist.",
          medicines: [
            { name: "Paracetamol", dosage: "500 mg", frequency: "After food, every 6-8 hrs as needed", duration: "3 days" },
        ...
        autoFill = {
          ...
          reason: "Substernal chest tightness with mild dyspnea and palpitations on moderate exertion.",
          diagnosis: "Suspected Angina Pectoris / Hypertensive episode. Advised urgent ECG and lipid profile.",
          treatment: "Avoid strenuous exertion. Low sodium diet. Follow-up post ECG and 2D Echo.",
          medicines: [
            { name: "Aspirin (Dispirin)", dosage: "75 mg", frequency: "Once daily with lunch", duration: "10 days" },
            { name: "Amlodipine", dosage: "5 mg", frequency: "Once daily in morning", duration: "14 days" }
          ]
        };
        ...
        autoFill = {
          ...
          reason: "Pruritic erythematous skin rash with mild swelling post suspected allergen exposure.",
          diagnosis: "Contact Dermatitis / Acute Urticarial Reaction.",
          treatment: "Avoid scratching, keep skin moisturized with gentle emollient. Avoid hot showers.",
          medicines: [
            { name: "Levocetirizine", dosage: "5 mg", frequency: "Once daily at night", duration: "5 days" },
            { name: "Calamine Lotion", dosage: "Topical application", frequency: "Twice daily to affected area", duration: "7 days" }
          ]
        };
        ...
        autoFill = {
          ...
          reason: "Throbbing unilateral headache accompanied by mild photophobia and morning dizziness.",
          diagnosis: "Tension-type headache / Mild migraine episode.",
          treatment: "Maintain regular sleep cycle, avoid screen glare, stay hydrated.",
          medicines: [
            { name: "Naproxen", dosage: "250 mg", frequency: "After food, SOS for acute headache", duration: "3 days" }
          ]
        };
    ...
    if (req.method === 'POST' && path === '/api/ai/suggest-booking') {
      ...
      if (text.includes('heart') || text.includes('chest') || text.includes('pressure') || text.includes('bp')) {
        department = "Cardiology";
      } else if (text.includes('skin') || text.includes('rash') || text.includes('acne') || text.includes('itch')) {
        department = "Dermatology";
      } else if (text.includes('brain') || text.includes('headache') || text.includes('migraine') || text.includes('nerve')) {
        department = "Neurology";
      ...
      const matchingDoctor = users.find(u => u.role === 'doctor' && (u.department === department || u.specialization === department)) || users.find(u => u.role === 'doctor');

      return send(res, 200, {
        status: "success",
        department,
        recommendedDoctorId: matchingDoctor?.id || null,
        recommendedDoctorName: matchingDoctor?.name || 'Dr. Ananya Rao',
    ...
    if (req.method === 'POST' && path === '/api/ai/suggest-rx') {
      ...
      let medicines = [
        { name: "Paracetamol", dosage: "500 mg", frequency: "After food, twice daily", duration: "3 days" }
      ];
      ...
      if (diag.includes('hypertension') || diag.includes('bp')) {
        ...
        medicines = [
          { name: "Amoxicillin + Clavulanic Acid", dosage: "625 mg", frequency: "Twice daily after meals", duration: "5 days" },
          { name: "Paracetamol", dosage: "650 mg", frequency: "SOS for fever > 100°F", duration: "3 days" },
          { name: "Pantoprazole", dosage: "40 mg", frequency: "Before breakfast, once daily", duration: "5 days" }
        ];
        ...
        medicines = [
          { name: "Levocetirizine", dosage: "5 mg", frequency: "Once daily at night", duration: "5 days" },
          { name: "Hydrocortisone cream 1%", dosage: "Thin layer", frequency: "Twice daily topically", duration: "7 days" }
        ];
    ...
    if (req.method === 'POST' && path === '/api/ai/summarize') {
      ...
      const mockSummary = {
        visit_summary: `Clinical Summary: ${notes}`,
        key_observations: "Patient vitals stable. Monitor blood pressure and hydration over the next 48 hours.",
        follow_up_reminder: "Schedule a follow-up consultation in 7 days or sooner if symptoms worsen."
      };
      
      return send(res, 200, { status: "success", data: mockSummary });
    }

    // ==========================================
    // FEATURE 4: EMERGENCY MEDICAL RECORD (QR & Profile)
    // ==========================================
    if (req.method === 'GET' && routeMatches(path, '/api/emergency/profile/:id')) {
      ...
      const profileData = {
        ...
        conditions: patient.conditions || 'Asthma',
...
if (require.main === module) {
  ...
  server.listen(PORT, HOST, () => console.log(`Medi Smart API running at http://${HOST}:${PORT}`));
}
