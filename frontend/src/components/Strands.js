...
function buildPalette(colors) {
  ...
  const Color = OGL.Color || function(hex) {
...
export function initStrands(container, options = {}) {
  ...
  const OGL = window.OGL;
  if (!OGL) {
    console.warn("OGL library not loaded yet.");
    return;
  }

  const { Renderer, Program, Mesh, Triangle, RenderTarget } = OGL;

  const props = {
    colors: options.colors || ["#F97316", "#7C3AED", "#06B6D4"],
...
export default function Strands(props) {
  ...
  useEffect(() => {
    const cleanup = initStrands(ctnDom.current, props);
    return cleanup;
  }, []);

  return React.createElement('div', {
    ref: ctnDom,
    className: 'strands-container ' + (props?.className || ''),
    style: props?.style
  });
}
